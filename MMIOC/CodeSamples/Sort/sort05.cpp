...
void heapify(int* tab, int heap_size1, int i1);
...
int tabHeap[N] = { 30,29,28,27,26,25,1,2,3,4,5,6,7,24,23,22,21,20,19,18,8,9,10,11,17,16,15,13,14,12 };
...
void heapify(int* tab, int heap_size1, int i1)
{
	...
	int iTab = tab[i];
	int rTab = tab[r];
	int largestTab = tab[largest];

	if (l <= heap_size && lTab > iTab)
	{
		largest = l;
	}		
	else {
		largest = i;
		largestTab = tab[largest];
	}
	if (r <= heap_size && rTab > largestTab)
	{
		largest = r;
	}
	if (largest != i)
